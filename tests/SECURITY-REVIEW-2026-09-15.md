...
## High: double-transfer on MagicBlock fallback (client)
...
If `buyInViaMagicBlock` fails with an ambiguous error (network timeout, partial success), the fallback sends a second transfer. Player could pay twice.

**Fix needed:** Before retry, check if the first transfer landed (query on-chain balance or transaction history). If ambiguous, mark as `needs_reconciliation` and let the server verify before retrying.
...
**No production changes.** Proposed migration needs integrated validation before apply.
